...
MoveitPlanner::MoveitPlanner(const rclcpp::NodeOptions &options)
: Node("hello_moveit", options), node_ptr(std::make_shared<rclcpp::Node>("example_moveit")), executor_ptr(std::make_shared<rclcpp::executors::SingleThreadedExecutor>())
{
  ...
  "arm_base_commands", 10, std::bind(&MoveitPlanner::topic_callback, this, std::placeholders::_1));
...
void MoveitPlanner::topic_callback(const interfaces::msg::ArmCmd & armMsg)
{
