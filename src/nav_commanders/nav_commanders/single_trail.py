...
class ReturnGoalAndStateAndPose:
    def __init__(self, pose: PoseStamped = PoseStamped(), goal: TrailGoal = TrailGoal.NO_GOAL, state: TrailState = TrailState.NONE_FOUND):
        self.goal_pose: PoseStamped = pose
        self.trail_goal_info: TrailGoal = goal
        self.trail_state_info: TrailState = state
...
class SingleCIRCTrail:
    ...
    def two_targets_close(self, sorted_point_array: PointArray, rover_pose: PoseStamped, curr_time: Time) -> TrailState:
        ...
        if len(self._curr_baking_points) <= 1:
            avg_point = sorted_point_array.points[0]
        else:
            avg_point = TrailHelperFunctions.average_of_points(self._curr_baking_points)

        # If not enough samples, add the closest point
        if len(self._curr_baking_points) < LogicTreeConstants.SAMPLES_TO_BAKE_TARGET and not sorted_point_array.is_moving[0]:
            self._curr_baking_points.append(sorted_point_array.points[0])
...
class TrailHelperFunctions:
    def sort_point_array(point_array: PointArray) -> PointArray:
        point_array.points.sort(key = lambda p: (p.x**2 + p.y**2 + p.z**2))
