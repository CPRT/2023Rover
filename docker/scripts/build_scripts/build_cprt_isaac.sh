...
PLATFORM="$(uname -m)"
IMAGE_NAME="cprt_isaac:2023Rover"
...
print_docker_build "Building Dockerfile.isaac_aarch64_cuda as isaac_aarch64_cuda from ros2_humble"
docker build -f "$DOCKERFILE_DIR"/Dockerfile.user \
    --network host \
    --build-arg BASE_IMAGE=ros2_humble:cprt_ros2_2023Rover \
    -t isaac_aarch64_cuda:cprt_ros2_2023Rover . \
    || exit 22

print_docker_build "Building Dockerfile.user as user from isaac_aarch64_cuda"
docker build -f "$DOCKERFILE_DIR"/Dockerfile.user \
    --network host \
    --build-arg BASE_IMAGE=isaac_aarch64_cuda:cprt_ros2_2023Rover \
...
print_docker_build "Building Dockerfile.cprt_2023Rover as final image named $IMAGE_NAME from zed:cprt_ros2_2023Rover"
